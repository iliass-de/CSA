...
DatabaseService.prototype.getReservedRooms = function (callback) {
    ...
    const sql = `SELECT Guest.email, Guest.first_name, Guest.last_name, Reservation.guest_id,
                    Reservation.room_id, Reservation.id as reservation_id, 
                    RoomType.room_type, Room.room_number, Reservation.check_out
                FROM Room
                LEFT JOIN Reservation ON Room.id = Reservation.room_id
                INNER JOIN RoomType ON Room.room_type_id = RoomType.id
                INNER JOIN Guest ON Reservation.guest_id = Guest.id;`;
...
DatabaseService.prototype.getNotReservedRooms = function (callback) {
    ...
    const sql = `SELECT Room.id as roomId, room_type, room_number
                FROM Room
...
DatabaseService.prototype.addReservation = function(guestId , roomId, check_in, check_out) {
    const self = this;
    const sql = `INSERT INTO Reservation (guest_id, room_id, check_in, check_out) VALUES ( 
                                    ?, ?, ?, ?);`;

    this.dbContext.serialize(function() {
        const stmt = self.dbContext.prepare(sql);
        stmt.run(guestId, roomId, check_in, check_out);
        stmt.finalize();
    });
};

DatabaseService.prototype.editReservation = function(guestId , roomId, check_in, check_out) {
    const self = this;
    const sql = `UPDATE Reservation SET room_id=?, check_in=?, check_out=? WHERE guest_id=?;`;

    this.dbContext.serialize(function() {
        const stmt = self.dbContext.prepare(sql);
        stmt.run(roomId, check_in, check_out, guestId);
        stmt.finalize();
    });
};

DatabaseService.prototype.deleteReservation = function(reservationId) {
    const self = this;

    const sql = `DELETE FROM Reservation WHERE guest_id=?;`;
